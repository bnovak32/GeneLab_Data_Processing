...
def create_header(assay_suffix, workflow_version):
    ...
    header += "Data were processed as described in GL-DPPD-7112-A "
    header += "(https://github.com/nasa/GeneLab_Data_Processing/blob/master/Microarray/Agilent_1-channel/Pipeline_GL-DPPD-7112_Versions/GL-DPPD-7112-A.md), "
    
    header += f"using NF_MAAgilent1ch version {workflow_version} "
    header += f"(https://github.com/nasa/GeneLab_Data_Processing/tree/NF_MAAgilent1ch_{workflow_version}/Microarray/Agilent_1-channel/Workflow_Documentation/NF_MAAgilent1ch)."
...
def create_annot_and_dge_section(organism, biomart_attribute, ensembl_version, 
    ...
    if not skip_DGE:
        ...
        de_step += "all groups were compared pairwise for each probe to generate a moderated t-statistic and associated p- and adjusted p-value."
    ...
    # Gene annotations
    annot_step = ""
    if custom_annot_source_name is None or 'agilent' in custom_annot_source_name:
...
def generate_protocol_content(args:argparse.Namespace, software_versions:dict, annotation_versions:dict, assay_suffix:str = "") -> str:
    ...
    description += f"Raw data quality assurance density, pseudo image, MA, and foreground-background plots were generated using limma (version {limma_version}), and boxplots were generated using ggplot2 (version {ggplot2_version}). "
