...
get_transcript_to_gene_mapping_from_ftp <- function(division, organism, ensembl_version, ensembl_genomes_portal = NULL) {
  ...
  #' NOTE: column positions (40 = ensembl_transcript_id, 14 = ensembl_gene_id)
  #' reflect the Ensembl mart-dump schema as specified; re-verify against a
  #' real file listing if this ever silently breaks after an Ensembl release.
  ...
  raw <- download_mart_dump(url)
  stopifnot("Expected >= 40 columns in transcript main table; Ensembl mart-dump schema may have changed" = ncol(raw) >= 40)
  raw %>% dplyr::transmute(ensembl_transcript_id = V40, ensembl_gene_id = V14) %>% dplyr::distinct()
}
...
if (organism %in% c("athaliana")) {
  ...
  if (!is.null(df_mapping)) {
    ...
    if (annot_type == 'agilent') {
      message(glue::glue("Using Agilent Ensembl transcript annotation, resolved to genes via Ensembl FTP mart dump"))
