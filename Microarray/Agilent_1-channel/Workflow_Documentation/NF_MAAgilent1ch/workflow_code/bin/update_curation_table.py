...
def parse_args():
    parser = argparse.ArgumentParser(
        ...
        description='Update Microarray Agilent-1channel assay table from ISA.zip with processed data file information.')
...
def add_raw_intensities_table_column(df, glds_prefix):
    ...
    raw_intensities = (f"{glds_prefix}_array_raw_intensities_GLmicroarray.csv")
...
def add_normalized_expression_table_column(df, glds_prefix):
    ...
    # Create the normalized expression filename - same for all samples
    normalized_expression = (f"{glds_prefix}_array_normalized_expression_GLmicroarray.csv")
    ...
    # Set the value for all rows (creates the column if it didn't exist)
    print(f"{'Updating' if existing_col else 'Adding'} column: {column_name}")
    df[column_name] = normalized_expression
