...
ch_dp_tools_plugin = params.dp_tools_plugin ? 
  ...
  : params.skipDE ? channel.value(file("$projectDir/bin/dp_tools__agilent_1_channel_skipDE")) : channel.value(file("$projectDir/bin/dp_tools__agilent_1_channel"))
...
"""
  ...
  PARSE_ANNOTATION_TABLE(params.annotation_file_path, organism_sci)
    ...
    ch_process_AGILE1CH = PROCESS_AGILE1CH.out.versions | map{ it -> it.text }
    ch_dp_tools_version = params.skipVV
      ? ( dp_tools_version ? dp_tools_version.map { it -> it.text} : channel.empty() )
      : ( VV_AGILE1CH.out.versions | map{ it -> it.text } )

    ch_software_versions = ch_software_versions
      | mix(ch_process_AGILE1CH)
